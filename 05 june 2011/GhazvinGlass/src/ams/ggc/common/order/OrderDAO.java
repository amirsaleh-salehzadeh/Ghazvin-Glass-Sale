...
public class OrderDAO extends BaseHibernateDAO implements OrderDAOInterface{
	...
	public OrderLST getOrderLST(OrderLST lst) throws AMSException {
			...
			try{
				...
				if(lst.getUsername()!=null&&!"".equalsIgnoreCase(lst.getUsername())){
					q = getSession().getNamedQuery("spGetOrdersList");
					q.setString(0, lst.getUsername());
					q.setInteger(1, lst.getFirst());
					q.setInteger(2, lst.getPageSize());
				}else{
					q = getSession().getNamedQuery("spGetOrdersSearchList");
					q.setString(0, "%"+lst.getKeyword()+"%");
					q.setInteger(1, lst.getFirst());
					q.setInteger(2, lst.getPageSize());
				}
	...
	public OrderENT saveEditOrder(OrderENT ent) throws AMSException {
		Session session = getSession();
		Transaction tx  = null;
		OrderENT ent2 = this.getOrderENT(ent);
	      try{
	    	 tx = session.beginTransaction();
	         ent2 = (OrderENT) session.merge(ent);
	         session.clear();
	         for (int i = 0 ; i < ent2.getDetailENTs().size() ; i++){
	        	 session.delete(ent2.getDetailENTs().get(i));
	         }
	         ArrayList<OrderDetailENT> orderDetailENTs = new ArrayList<OrderDetailENT>();
	         for (int i = 0 ; i < ent.getDetailENTs().size() ; i++){
	        	 OrderDetailENT detailENT = new OrderDetailENT();
	        	 detailENT = ent.getDetailENTs().get(i);
	        	 detailENT.setOrderId(ent2.getOrderId());
	        	 orderDetailENTs.add((OrderDetailENT) session.merge(detailENT));
	         }
	         ent2.setDetailENTs(orderDetailENTs);
	         tx.commit();
	         session.clear();
	         session.close();
		} catch (HibernateException e) {
