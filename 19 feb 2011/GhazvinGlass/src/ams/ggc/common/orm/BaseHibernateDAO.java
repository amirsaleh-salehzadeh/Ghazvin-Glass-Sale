...
import ams.ggc.HibernateSessionFactory;
...
public class BaseHibernateDAO implements IBaseHibernateDAO {
	...
	public Object saveWithEx(Object ent) throws AMSException {
		try {
			return save(ent, null, null);
		} catch (Exception ex) {
	...
	public void refreshRelatedObjects(Object ent, String identifierFied,
		...
		try {
			...
			if (identifierFied != null && !"".equals(identifierFied)) {
				try {
					id = (Integer) AMSUtililies.invoke(ent, identifierFied);
					id = NVL.getInteger(id);
					AMSUtililies.invoke(ent, identifierFied, id);
	...
	public void deleteSelectedIds(Class clazz, String selectedIds) {
		String[] ids = AMSUtililies.splitSelectedIds(selectedIds, ",");
	...
	public AMSException getAIPException(Exception ex) {
		return getAMSException(AIPEX_UNKNOWN, ex);
	}
