...
public class orderBusinessManager extends BusinessManagerSuper {
	...
	public OrderLST getOrderLST(OrderLST lst) {
		...
		try {
			...
			while (rs.next()) {
				...
				orderENT.setTotalPrice(rs.getString("price"));
	...
	public OrderENT saveOrder(OrderENT ent){
		try {
			PreparedStatement ps;
			ps = cn.prepareStatement("insert into orders (ordertext, productname, username, date, status, size, amount, thickness, packing, price) values(?,?,?,?,?,?,?,?,?,?)");
			...
			ps.setString(10, ent.getTotalPrice());
			ps.execute();
