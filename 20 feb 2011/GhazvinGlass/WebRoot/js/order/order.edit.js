function addToListAUser(){
	$("#username").val($("input[name=user]:checked").val());
}
...
function showDialogSearchUser(){
	var loader = "user.do?reqCode=userList4Order";
	showDialog('dlgUserList','center',loader);
}
function goToSearchUser(){
	$('form#listForm').ajaxSubmit(function(data){
		$("#dlgUserList_content_ajax").html(data);
	});
}
...
function saveEdit(){
	...
	$("#editReqCode").val("saveEdit");
	$("form#orderEditForm").submit();
}
function saveNewOrder(){
	if($('#orderList >tbody >tr').length > 1 && $("#username").val()!==""){
		$(".packingChx").each(function(){
			if($(this).attr("checked")!==true){
				$(this).val("0")
			}
